...
class EncoderClassifier(nn.Module):
    ...
    def freeze_classifier(self):
        for _param in self.classifer.parameters():
            _param.require_grad = False

    def unfreeze_classifier(self):
        for _param in self.classifier.parameters():
            _param.require_grad = True
...
def run(params):
    ...
    _model_save_path = os.path.join(params["save_path"], params['dataset'], "auto", str(time.time()))
    ...
    for e in range(params['epoch']):
        # if (e % 2 == 0):
        #     _Model.freeze_encoder()
        #     _Model.unfreeze_classifier()
        # else:
        #     _Model.freeze_classifier()
    ...
    torch.save(_Model.encoder, os.path.join(_model_save_path, "model_encoder.pt") )
